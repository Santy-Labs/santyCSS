...
### Fixed
- **The icon stylesheet was never published.** `santy-icons.css` existed only at
  the repo root — it was not generated by `build.js`, not mirrored into `dist/`,
  not listed in `package.json` `files[]`, and had no export. So
  `npm install santycss` shipped no icons at all, and the CDN URL the docs give
  Webflow users —
  `https://cdn.jsdelivr.net/npm/santycss@2/dist/santy-icons.css` — returned
  **404**. Every npm and CDN consumer got a page with no icons.
  The file is now emitted to `dist/`, published in `files[]`, and exported as
  `santycss/css/icons`.
  - It is deliberately **not** run through the `prefix` transform: the file is
    1.3MB of `url("data:image/svg+xml,…")` values and the prefixer only shields
    `url(...)` up to the first `)`, so a prefixed build could corrupt the
    embedded SVGs. Icon class names are published verbatim in the docs anyway.

### Added
- Regression tests covering the failure modes above: `dist/santy-icons.css`
  exists and matches source, the package actually publishes it, the base
  classes and `--icon-url` / `--brand-icon-url` custom properties are present,
  and **every icon `icons.html` renders resolves to a real CSS rule** (2,191
  checked).
- A completeness guard on `icons.html` — it must end with `</html>`, have
  balanced block comments, and every inline script must parse. The build
  deployed to production was truncated mid-comment, which made the whole inline
  script a syntax error, left the grids empty and caused all 2,191 icons to
  disappear. This test fails on that exact file.
